...
export default function Input({
    ...
    return (
        <>
            <div className={cn(className, "relative flex flex-col m-4 rounded-lg hover:opacity-90")}>
                <input
                    // On met TOUT dans le cn() pour éviter les concaténations bizarres
                    className={cn(
                        // Styles de base communs
                        "border px-5 border-gray-50/20 focus-visible:outline-none",

                        // État d'erreur
                        error && "border-red-500/50 ring-red-500/10 focus-visible:ring-red-500/30 outline-2 outline-red-600",

                        // Cas spécifique au submit
                        type === "submit" && "flex items-center px-2 cursor-pointer "
                    )}
                    type={type}
